...
if [[ $PROJECT_IDS == "" ]]; then
    secure_read "Project ID(s)" PROJECT_IDS
fi;
...
if [[ $CSV == "True" ]]; then
    echo "\"PROJECT_ID\", \"PROJECT_NAME\", \"PROJECT_OWNER\", \"PROJECT_APPLICATION\", \"INSTANCE_NAME\", \"ENABLED_SERIAL_PORTS\", \"ENABLED_SERIAL_PORT_LOGGING\", \"ENABLED_SERIAL_PORTS_STATUS_MESSAGE\", \"ENABLED_SERIAL_PORT_LOGGING_STATUS_MESSAGE\", \"VIOLATION\"";
fi;
    ...
    if [[ $INSTANCES != "[]" ]]; then
        ...
        echo $INSTANCES | jq -rc '.[]' | while IFS='' read -r INSTANCE;do
            ...
            if [[ $ENABLED_SERIAL_PORTS == "true" ]]; then
                ENABLED_SERIAL_PORTS_STATUS_MESSAGE="Serial port enabled";
                VIOLATION="Serial port enabled";
            fi;
            ...
            if [[ $ENABLED_SERIAL_PORT_LOGGING != "true" ]]; then
                ...
                VIOLATION="Serial port logging disabled";
            fi;
