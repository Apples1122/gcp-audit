	...
	echo $CLOUD_ARMOR_POLICIES | jq -r -c '.[]' | while IFS='' read -r POLICY; do
		...
		if [[ $CSV == "True" ]]; then
			echo $CLOUD_ARMOR_POLICY_RULES | jq -r -c '.[]' | while IFS='' read -r RULE; do
                ...
                echo "\"$PROJECT_ID\", \"$PROJECT_APPLICATION\", \"$PROJECT_OWNER\" \"$CLOUD_ARMOR_POLICY_NAME\",\"$RULE_DESCRIPTION\",\"$RULE_ACTION\",\"$RULE_MATCH\",\"$CLOUD_ARMOR_POLICY_DDOS_PROTECTION_ENABLED\"";
			done;
		else
		    ...
		    echo $CLOUD_ARMOR_POLICY_RULES | jq -r -c '.[]' | while IFS='' read -r RULE; do
                ...
                echo "Rule Match: $CLOUD_ARMOR_POLICY_RULES
		    done;
		    echo $BLANK_LINE;
		fi;
	done;
